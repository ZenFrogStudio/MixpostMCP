...
import PostPreviewInstagram from "@/Components/PostPreview/PostPreviewInstagram.vue"
...
// An option the user never touched is not written to the version, so the preview falls back to the
// same schema default PostOptions.vue shows in the composer. Without this an untouched LinkedIn
// audience or YouTube visibility would preview as blank rather than as what will actually publish.
const resolveProviderOptions = (account, version) => {
    const saved = version?.options?.[account.provider] ?? {};

    return (account.post_options ?? []).reduce((options, field) => {
        options[field.key] = saved[field.key] !== undefined ? saved[field.key] : field.default;
...
const previews = computed(() => {
    return props.accounts.map((account) => {
        ...
        const version = accountVersion ? accountVersion : defaultVersion.value;

        return {
            account,
            content: version.content,
            options: resolveProviderOptions(account, version),
            providerComponent: {
